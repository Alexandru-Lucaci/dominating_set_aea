...
def improve_solution(graph, initial_solution):
    ...
    while improved:
        ...
        for v in list(solution):
            # Check if v can be removed
            temp_solution = solution - {v}
            
            # Check if temp_solution is still dominating
            is_valid = True
            for u in range(graph.n):
                if u not in temp_solution:
                    # u must have a neighbor in temp_solution
                    has_dominator = False
...
def local_search_improvement(graph, initial_solution, time_limit=10):
    ...
    while time.time() - start_time < time_limit:
        ...
        # Try 3-opt if no 2-opt improvement
        if not improved and iteration < 5:  # Limit expensive 3-opt
            for v1 in list(best_solution)[:20]:  # Limit search
                if time.time() - start_time > time_limit:
                    break
...
def is_valid_dominating_set(graph, solution):
    ...
    for v in range(graph.n):
        if v not in solution_set:
            # Check if v has a neighbor in solution
            has_dominator = False
...
def solve_dominating_set_optimized(graph, time_limit=300, debug=False):
    ...
    # Step 1: Try multiple strategies
    strategies = [
    ...
    # Allocate time to different strategies
    time_per_strategy = time_limit / len(strategies) * 0.7  # 70% for all strategies
    improvement_time = time_limit * 0.3  # 30% for improvement
